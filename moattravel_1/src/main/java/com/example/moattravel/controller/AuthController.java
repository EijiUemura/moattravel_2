...
public class AuthController {
	...
	@GetMapping("/signup")
	public String ssignup(Model model) {
		model.addAttribute("signupForm", new SignupForm());
		return "auth/signup";
	}
	
	@PostMapping("/signup")
	public String singnup (@ModelAttribute @Validated SignupForm signupForm, 
			BindingResult bindingResult, RedirectAttributes redirectAttributes,HttpServletRequest httpServletRequest) {
		if(userService.isEmailRegistered(signupForm.getEmail())) {
			FieldError fieldError = new FieldError(bindingResult.getObjectName(),"email", "すでに登録済みです");
		...
		redirectAttributes.addFlashAttribute("succesMessage","ご入力いただいたメールアドレスに認証コードを送信しました。メールに記載されているリンクをクリックし、会員登録を完了してください。");
		return "redirect:/";
	}
	...
	public String verify(@RequestParam(name = "token") String token,Model model) {
		...
		if(verificationToken != null) {
			...
			String succesMessage = "会員登録が完了しました。";
